...
export default async function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  if (profile.role === "partner") redirect("/partner/overview");
  if (!ALLOWED_ROLES.includes(profile.role as AllowedRole)) redirect("/unauthorized");

  return (
    <DefaultPageLayout>
      <Suspense fallback={null}><OrderDetailsDrawerHost /></Suspense>
      {children}
    </DefaultPageLayout>
  );
}
