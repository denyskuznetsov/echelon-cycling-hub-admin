...
### AD-11 — [ADOPTED] Sync results describe one proven run
...
- **Rule:** Use the same run record for scope, interval, completion, counters and result state. A successful selected-period result means enumeration completed and every required candidate reconciled successfully for that recorded interval; it is not an atomic source snapshot or proof of future webhook delivery. Keep in-progress, failed, incomplete, stale and locally empty states distinct. User-facing progress, result and error language follows Workshop patterns; do not introduce a separate “confidence” status or success vocabulary. Retain failure details and explicit resume/restart. Label old reserved-only runs only according to their original scope; never pair `last_success_at` with an unrelated latest run's scope. Show recorded dates when the selected view changes, and do not assert coverage of a wider/different range. Retries replace an order outcome rather than inflate success totals; skipped rows do not count as successfully refreshed orders. A deleted/unavailable candidate remains an explicit failure until existing recovery semantics resolve it, not silently removed or declared complete.
...
| CAP-8 selected-period refresh/recovery | Existing worker extended with fixed run scope | AD-4, AD-10, AD-11, AD-12 |
